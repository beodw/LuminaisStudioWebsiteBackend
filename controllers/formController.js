...
module.exports.FormSubmission = async (event) => {
  try {
    connectDB(); // Connect to the database
    ...
    const savedRecord = await formAnswer.save(); 
    
    
    // Send confirmation email
    // await sendEmail(email, 1000, "SLE", "Monime.io");

    // Return the generated submissionId to the frontend
    return {
      statusCode: 200,
      body: JSON.stringify({
        message: "Form processed successfully",
        recordId: savedRecord._id, // Send the _id in the response
      }),
      headers: {
        "Access-Control-Allow-Origin": "*", // Allow all origins
        "Access-Control-Allow-Headers": "Content-Type, Authorization", // Include required headers
